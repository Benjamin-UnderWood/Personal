...
var queryFromObject = function(param) {
    ...
    */
    var pairs = []
    var keys = Object.keys(param) // 可枚举的实例属性(对象属性)；没有顺序
    for (var i = 0; i < keys.length; i++) {
        var k = keys[i]
        var p = k + '=' + param[k] // param[k]为属性值
        pairs.push(p)
    }
    return pairs.join('&')
}
...
Object.keys(obj) 可枚举的实例(对象)属性
Object.getOwnPropertyNames(obj) 实例的所有属性(可枚举 + 不可枚举)
for-in 所有能够通过对象访问的可枚举的(既包括存在于实例中的属性， 也包括存在于原型中的属性
有时候会出现不想要的东西，需要验证)
...
var ajax = function(request) {
    ...
    */
    ...
    // 如果有 Content-Type 就设置没有就不设置(看传入的数据决定)
...
var r = {
    ...
    callback: function(response) {
        ...
        // 成功返回
        if (res.success) {
            // 绑你点到主页去(登录成功需要自己去那个主页)
            // ajax登录成功了，页面的改动需要自己来
            // 就是一直强调的，后端服务器告诉你成功，或者删除什么的，前端页面的显示需要自己用函数改掉！！！！
            // 前端是显示，后端是数据库
