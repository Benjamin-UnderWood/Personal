...
$(function () {
    // 常用功能
    $("#btn_show_dialog").on("click", showDialog);
    $(".dlg_btn_close").on("click", hideDialog);    
    $("#dlg_submit").on("click", submitHandler);

    // 移动
    $("#dialog").on("mousedown", moveHandler);

    // 最大化 || 还原
    $(".dlg_btn_max_top").on("click", maxDialog);
    $(".dlg_btn_reback_top").on("click", rebackDialog);
});
